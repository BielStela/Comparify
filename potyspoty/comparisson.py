...
class Comparator():
    ...
    def fit(self, bands_features: List[SongFeatures]):
        ...
        self.features_all, self.band_names_long = self.concat_(self.bands_features)
    ...
    def plot_plotly(self):
        ...
        for i, df in enumerate(self.bands_features):
            trace0 = go.Scatter(
                x = self.projected_features[np.array(self.band_names_long)==df.band_name, 0],
                y = self.projected_features[np.array(self.band_names_long)==df.band_name, 1],
                mode= 'markers',
                marker= dict(size= 8,
...
def most_similar_song(band1:SongFeatures, band2:SongFeatures) -> pd.DataFrame:
